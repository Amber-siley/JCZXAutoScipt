...
**任何新增特性（功能、工具、方法、配置解析行为等）都必须配套编写单元测试**，与特性代码一起提交，不允许只在本地手动验证。这是硬性要求。

- **测试放哪**：按代码性质选择 `tests/pure/`（纯逻辑）、`tests/engine/`（引擎级，`FakeMatcher`/`FakeToken` 桩替身）、`tests/regression/`（配置解析/保存锁行为）。
- **测试要能独立跑**：新增测试应可独立执行，不依赖设备连接（用桩替身）、不污染真实配置（用 `real_config_dir` 只读副本 fixture）。
- **改动必须可验证**：单测应覆盖特性**新增的行为**，能验证特性按预期工作——遵循 AGENTS.md 的"目标驱动执行"，把"新增功能"转化为"为新增行为写测试并让它通过"。
- **运行**：新增测试通过后，再跑 `uv run pytest` 确认不破坏既有测试。
